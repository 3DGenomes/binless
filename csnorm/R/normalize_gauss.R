...
csnorm_gauss_signal = function(cs, verbose=T, ncores=ncores, tol=1e-3) {
  ...
  params = foreach(g=groupnames, .combine=rbind) %dopar%
    csnorm:::csnorm_fused_lasso(mat[name==g], cs@settings$trails, positive=T, zero.corner=(cs@settings$circularize<=0),
  ...
  mat[,phi:=value]
  #ggplot(mat)+facet_wrap(~name)+geom_raster(aes(bin1,bin2,fill=phi))+geom_raster(aes(bin2,bin1,fill=phihat))+scale_fill_gradient2()
