...
transformed data {
  ...
  int Xv[nnz(N)];
  int Xu[N+1];
  row_vector[K] p; //projector on centering constraint
...
transformed parameters {
  ...
  vector[K-2] beta_diff;
  vector[N] pred; //spline interpolant
  {
    vector[K] beta_aug;
    beta_aug[1] = 0;
    beta_aug[2:] = beta;
    beta_centered = beta_aug - (p * beta_aug) * rep_vector(1,K);
  }
  beta_diff = beta_centered[:(K-2)]-2*beta_centered[2:(K-1)]+beta_centered[3:];
  pred = intercept + csr_matrix_times_vector(N, K, Xw, Xv, Xu, beta_centered);
}
model {
  //exponential GAM
  y ~ normal(pred, sigma);
  ...
  beta_diff ~ normal(0, 1/lambda);
  lambda ~ cauchy(0,1);
}
generated quantities {
  matrix[N,K] designmat;
  designmat = csr_to_dense_matrix(N, K, Xw, Xv, Xu);
}
