...
functions {
  ...
  matrix design(vector x, int K, int q, int t) {
    return bspline(x, K-q, q) * scam_sigma(K,t);
  }
  
  // difference operator for that design matrix
  matrix difference_op(int K, int t) {
...
parameters {
  real beta_scam;
  vector[K-1] beta_scam_increments;
  real<lower=0> sigma2;
  real<lower=0> lambda;
}
transformed parameters {
  vector[K] betatilde;
  vector[K] beta;
  betatilde[1] <- beta_scam;
  betatilde[2:K] <- exp(beta_scam_increments);
  beta[1] <- beta_scam;
  beta[2:K] <- beta_scam_increments;
}
model {
  //exponential GAM
