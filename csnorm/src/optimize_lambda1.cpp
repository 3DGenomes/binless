...
double obj_lambda1::operator()(double const& x) const { return get(std::pow(10,x))["BIC"]; }
...
NumericVector obj_lambda1::get(double const& lambda1) const {
    ...
    return NumericVector::create(_["eCprime"]=0, _["lambda1"]=lambda1, _["BIC"]=BIC, _["dof"]=dof);
}

NumericVector refine_minimum(const obj_lambda1& obj, double lam1, double lam1_min, int refine_num, NumericVector patchvals, bool positive) {
...
NumericVector cpp_optimize_lambda1(const DataFrame mat, int nbins, double tol_val, bool positive,
  ...
  if (as<double>(cl["no"]) == 1)
    return NumericVector::create(_["eCprime"]=0, _["lambda1"]=std::abs(patchvals(0)), _["dof"]=0,
  ...
  if (constrained) {
    if (positive) {
      ...
      //Rcout << "  set lmin= " << lmin << " and lmax= " << lmax << std::endl;
    } else {
  ...
  NumericVector retval = refine_minimum(obj, lam1, lmin, refine_num, patchvals, positive);
  std::clock_t c_in3 = std::clock();
  //Rcout << "  opt lambda1 between " << lmin << " and " << lmax << ", found minimum at " << lam1 << " refined at "
  //      << as<double>(retval["lambda1"]) << std::endl;
