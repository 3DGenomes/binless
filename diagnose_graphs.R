...
g.R = binless:::compute_2d_connectivity_graph(nbins)
plot(g.R)
data=CJ(bin1=1:nbins,bin2=1:nbins)[bin2>=bin1]
data[,value:=(bin1+bin2)%/%2]
ggplot(data)+geom_raster(aes(bin1,bin2,fill=value))

binless:::boost_build_patch_graph_components(nbins,data[order(bin2)],1e-3)

a=binless:::boost_build_patch_graph_components(nbins,data,1e-3)
b=binless:::build_patch_graph(data,binless:::gfl_compute_trails(nbins),1e-3)
all(a$membership==b$components$membership-1)

load("tmp_csig.RData")
bic_r = binless:::gfl_BIC(csig, .1, 10, -0.1)
ggplot(bic_r$mat)+geom_raster(aes(bin1,bin2,fill=valuehat))+geom_raster(aes(bin2,bin1,fill=value))+scale_fill_gradient2()

bic_c = binless:::get_bic(csig, .1, 10, -0.1)
...
b=foreach (lambda2=5:15,.combine=rbind) %do% {
  bic_r = binless:::gfl_BIC(csig, .1, lambda2, -0.1)
  bic_c = binless:::get_bic(csig, .1, lambda2, -0.1)
...
csig.r = binless:::optimize_lambda1_eCprime(matg, csig, constrained=T)
ret.c = binless:::cpp_optimize_lambda1_eCprime(matg, csig@settings$nbins, csig@settings$tol.val, T, 0.05)
