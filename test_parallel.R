...
#matg[,value.ref:=binless:::gfl_get_value(valuehat, weight, trails, .1, 1, 0)]
...
matg[,value:=binless:::gfl_get_value(valuehat, weight, trails, .1, 1, 0, nthreads=10)]
...
bench=function(x){matg[,value:=binless:::gfl_get_value(valuehat, weight, trails, .1, 1, 0, nthreads=x)]}
jobs = lapply(1:20, function(s) local({s = s; bquote(bench(.(s)))}) )
...
binless:::gfl_BIC(matg, trails, .1, 1, 0, tol.value=1e-3, ncores=4)

lambda2 = binless:::optimize_lambda2(matg, trails, tol=1e-3, ncores=4)
profvis({vals = binless:::optimize_lambda1_eCprime(matg, trails, tol=1e-3, lambda2=1, positive=T, ncores=1)})
vals = binless:::optimize_lambda1_only(matg, trails, tol=1e-3, lambda2=1, positive=T, ncores=30)
