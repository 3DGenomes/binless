...
for (resolution in c(5000,20000,50000)) {
...
if (F) {
  ...
  info = foreach(run=c("cs5","cs5b","cs10","cs20"),cs=c(cs5,cs5b,cs10,cs20), .combine=rbind) %do% {
    data.table(run=run,qmin=cs@settings$qmin,dfuse=cs@settings$dfuse,base.res=cs@settings$base.res,
               cv=binless:::has_converged(cs), resolutions=list(sapply(cs@groups,function(x){x@resolution})))
  }
  ...
  ggsave(filename=paste0("images/rao_",sub,"_4resolutions_base10_binned.png"),width=32,height=20)
  ...
  ggsave(filename=paste0("images/rao_",sub,"_GMvsIMR90_base10at5_observed.png"),width=10,height=5)
  ...
  ggplot(mat.sig)+
    ...
    theme_void()+ theme(axis.title=element_blank(),
                        panel.background = element_rect(fill = "white", colour = "black"),
                        panel.spacing=unit(0,"cm")) + guides(fill=F)
  ggsave(filename=paste0("images/rao_",sub,"_GMvsIMR90_base10at5_binned_normalized.png"),width=10,height=5)
  ...
  ggplot(mat)+
    ...
    theme_void()+ theme(axis.title=element_blank(),
                        panel.background = element_rect(fill = "white", colour = "black"),
                        panel.spacing=unit(0,"cm")) + labs(fill="log10 FC")
  ggsave(filename=paste0("images/rao_",sub,"_GMvsIMR90_base10at5_binned_difference.png"),width=8,height=5)
  ...
  ggplot(mat)+geom_raster(aes(begin1,begin2,fill=delta/log(10)))+
    ...
    theme_void()+ theme(axis.title=element_blank(),
                        ...
                        panel.spacing=unit(0,"cm"))+
    coord_fixed()+labs(fill="log10 FC")
  ggsave(filename=paste0("images/rao_",sub,"_GMvsIMR90_base10at5_binless_differences.png"),width=8,height=5)
