...
model {
  //exponential GAM
  y ~ normal(intercept + csr_matrix_times_vector(N, K, Xw, Xv, Xu, beta_centered), sigma2);
  ...
  beta_centered[:(K-2)]-2*beta_centered[2:(K-1)]+beta_centered[3:] ~ normal(0, sigma2/lambda);
}
