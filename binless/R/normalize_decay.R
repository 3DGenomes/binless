...
gauss_decay_optimize = function(csd, design, Kdiag, original_lambda_diag,
  ...
  for(uXD in unique(XD)) {
    ...
    nobs_vec=csd[,nobs][cbegin[1]:(cbegin[2]-1)]
    ...
    if (Dsets > 1) {
      for (d in 2:Dsets) {
        ...
        nobs_vec=c(nobs_vec,csd[,nobs][cbegin[1]:(cbegin[2]-1)])
  ...
  dmat=csd[,.(name,dbin,distance,kappahat,std,nobs,log_decay=decay_out$log_decay)]
