...
load("data/rao_HiCall_SEMA3C_1M_csnorm_optimized_base10k_bpk30_dfuse20_cpp.RData")
sub="SEMA3C_1M"
...
for (resolution in c(5000,20000)) {
  ...
  ggplot(mat)+
    ...
    theme_void()+theme(legend.position = "none", axis.title=element_blank())
...
#Binless differences
mat=get_interactions(cs, resolution=resolution, group="all", type="CSbdiff", ref=cs@experiments[1,name])
ggplot(mat)+
