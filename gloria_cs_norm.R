...
#load data
biases=fread("gloria_biases_wt1_chr19.dat", stringsAsFactors = T)
data=fread("gloria_data_wt1_chr19.dat", stringsAsFactors = T)
...
system.time(subop <- optimizing(sm, data = list(N=subdata[,.N], K=10, B=3, #dangling L/R and rejoined
...
system.time(op <- optimizing(sm, data = list(N=data[,.N], K=10, B=3, #dangling L/R and rejoined
...
system.time(fit <- gam(N ~ s(log(distance),bs="ps", m=c(2,2), k=10) +
                         category:(log(dangling.L.1) + log(dangling.R.1) + log(rejoined.1) +
                         log(dangling.L.2) + log(dangling.R.2) + log(rejoined.2)),
                       data=subdata, family=nb()))
