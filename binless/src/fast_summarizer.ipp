...
void Summarizer<SummarizerImpl>::set_poisson_lsq_summary(const std::vector<double>& log_expected_std, const FastSignalData& data, double pseudocount) {
  ...
  //center estimate
  double avg = SummarizerImpl::get_settings().get_nobs().dot(estimate)/SummarizerImpl::get_settings().get_nobs().sum();
  estimate = (estimate.array() - avg).matrix();
  //compute weight
  Eigen::VectorXd weight = (estimate.array().exp()*SummarizerImpl::get_settings().get_nobs().array()).matrix();
  /*Rcpp::Rcout << "BEFORE\n";
  Rcpp::Rcout << "distance phihat weight sum_obs sum_exp\n";
  Rcpp::Rcout << (Eigen::MatrixXd(estimate.rows(),5) << SummarizerImpl::get_settings().get_log_distance().array().exp().matrix(),
                  estimate, weight, sum_obs, sum_exp).finished();*/
  //report back
  SummarizerImpl::get_summary().set_phihat(estimate);
  SummarizerImpl::get_summary().set_weight(weight);
  Rcpp::Rcout <<"set_poisson_lsq_summary for decay " << estimate.rows() << " " << weight.rows() << "\n";
  Rcpp::Rcout << SummarizerImpl::get_summary().get_phihat().rows() << "\n";
}

template<class SummarizerImpl>
void Summarizer<SummarizerImpl>::update_summary(const ResidualsPair& z, const Eigen::VectorXd& estimate) {
  //compute weight
  ...
  Rcpp::Rcout << (Eigen::MatrixXd(phihat.rows(),3) << SummarizerImpl::get_settings().get_log_distance().array().exp().matrix(), phihat,
                  weight_sum).finished();*/
  SummarizerImpl::get_summary().set_phihat(phihat);
  SummarizerImpl::get_summary().set_weight(weight_sum);
}
