...
bf_per_kb=signif(10^(seq(-0.7,0.7,length.out = 9)),digits=2)
...
foreach (bpk=bf_per_kb) %dopar% {
  ...
  save(cs, file=paste0("data/rao_HiCall_chrX_150k_bfpkb",bpk,"_csnorm_optimized.RData"))
}

#re-bin
registerDoParallel(cores=30)
foreach (bpk=bf_per_kb) %dopar% {
  load(paste0("data/rao_HiCall_chrX_150k_bfpkb",bpk,"_csnorm_optimized.RData"))
  cs@binned=list()
...
outputs = foreach (i=bf_per_kb,.combine=rbind) %do% {
  load(paste0("data/rao_HiCall_chrX_150k_bfpkb",i,"_csnorm_optimized.RData"))
...
#nu and delta: init
nu.init = foreach (i=bf_per_kb,.combine=rbind) %do% {
  load(paste0("data/rao_HiCall_chrX_150k_bfpkb",i,"_csnorm_optimized.RData"))
...
nu.init[,dset:=ordered(dset,levels=bf_per_kb)]
ggplot(nu.init[,.(dset,pos,nu=exp(log_nu))])+ylim(0,2)+
...
#nu
ggplot(nu[,.(dset,pos,nu=exp(log_nu))])+geom_line(aes(pos,nu),colour="black")+facet_wrap(~dset)+ylim(0.2,5)
