...
#
ggplot(nu.init[,.(dset,pos,delta=exp(log_delta))])+scale_y_log10()+geom_line(aes(pos,delta),colour="black")+facet_grid(~dset)+coord_flip()
...
#nu*delta
ggplot(nu[,.(dset,pos,nu=exp(log_nu+log_delta))])+scale_y_log10(limits=c(0.1,10))+geom_line(aes(pos,nu),colour="black")+
  facet_grid(~dset)+coord_flip()
#delta
