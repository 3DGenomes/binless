...
List wgfl_diff_perf_warm(const DataFrame cts, const DataFrame ref, double dispersion, int nouter, int nbins,
                         ...
                         double lam1, double lam2, double alpha, double inflate, int ninner, double converge,
                         int diag_rm, NumericVector phi_ref_i, NumericVector beta_i)
{
  ...
  return List::create(_["beta"]=wrap(beta_r), _["alpha"]=wrap(alpha), _["phi.ref"]=wrap(phi_ref_r), _["phi"]=wrap(delta_r), _["mat"]=mat,
