...
#' @keywords internal
#' 
csnorm_gauss_decay_muhat_data_perf = function(cs, pseudocount=1e-2) {
  #bin distances
...
#' @keywords internal
#' 
csnorm_gauss_decay_muhat_mean_perf = function(cs) {
...
#' @keywords internal
#' 
csnorm_gauss_decay_perf = function(cs, verbose=T, init.mean="mean", init_alpha=1e-7) {
  if (init.mean=="mean") {
  ...
  op=optimize_stan_model(model=csnorm:::stanmodels$gauss_decay_perf, data=data, iter=cs@settings$iter,
...
#' @keywords internal
#' 
csnorm_gauss_genomic_muhat_data_perf = function(cs, pseudocount=1e-2) {
...
#' @keywords internal
#' 
csnorm_gauss_genomic_muhat_mean_perf = function(cs) {
  #compute bias means
  init=cs@par
  bsub=copy(cs@biases)
  bsub[,c("log_iota","log_rho"):=list(init$log_iota,init$log_rho)]
...
#' @keywords internal
#'   
csnorm_gauss_genomic_perf = function(cs, verbose=T, init.mean="mean", init_alpha=1e-7) {
  if (init.mean=="mean") {
  ...
  op=optimize_stan_model(model=csnorm:::stanmodels$gauss_genomic_perf, data=data, iter=cs@settings$iter,
                         verbose=verbose, init=0, init_alpha=init_alpha)
...
#' @keywords internal
#' 
csnorm_gauss_dispersion_perf = function(cs, counts, weight=cs@design[,.(name,wt=1)], verbose=T, init_alpha=1e-7) {
  Kdiag=round((log10(cs@settings$dmax)-log10(cs@settings$dmin))* cs@settings$bf_per_decade)
  bbegin=c(1,cs@biases[,.(name,row=.I)][name!=shift(name),row],cs@biases[,.N+1])
...
#' @keywords internal
#' 
has_converged_perf = function(cs) {
...
#' Cut-site normalization (normal approximation, performance iteration)
#' 
...
#' 
#' @examples
plot_diagnostics_perf = function(cs) {
